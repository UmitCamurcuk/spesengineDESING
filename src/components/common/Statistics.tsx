  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-foreground">
            {t('statistics.generic.title') || 'İstatistikler ve Analiz'}
          </h3>
          <p className="text-sm text-muted-foreground">
            {t('statistics.generic.subtitle') || 'Kullanım istatistikleri ve performans metrikleri'}
          </p>
        </div>
        <Badge variant="primary" size="sm">
          <BarChart3 className="h-3 w-3 mr-1" />
          {t('statistics.generic.badge_live') || 'Canlı Veri'}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card padding="md">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">
                {t('statistics.generic.total_usage') || 'Toplam Kullanım'}
              </p>
          ...
          <div className="flex items-center mt-2">
            ...
            <span className="text-sm text-emerald-500 dark:text-emerald-400">
              {t('statistics.generic.change_from_last_month', { percent: 12 }) ||
                `+${12}% geçen aya göre`}
            </span>
          </div>
        </Card>
        ...
        <Card padding="md">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">
                {t('statistics.generic.active_items') || 'Aktif Kayıtlar'}
              </p>
          ...
          <div className="flex items-center mt-2">
            <span className="text-sm text-muted-foreground">
              {(() => {
                const percent = Math.round((fallbackStats.activeCount / fallbackStats.totalCount) * 100);
                return (
                  t('statistics.generic.of_total', { percent }) ||
                  `Toplamın %${percent}'i`
                );
              })()}
            </span>
        ...
        <Card padding="md">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">
                {t('statistics.generic.this_month') || 'Bu Ay'}
              </p>
          ...
          <div className="flex items-center mt-2">
            <span className="text-sm text-muted-foreground">
              {t('statistics.generic.updates_made') || 'Güncelleme sayısı'}
            </span>
          </div>
        </Card>
        ...
        <Card padding="md">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">
                {t('statistics.generic.last_used') || 'Son Kullanım'}
              </p>
              <p className="text-sm font-bold text-foreground">
                {fallbackStats.lastUsed
                  ? new Date(fallbackStats.lastUsed).toLocaleDateString()
                  : t('statistics.generic.never') || 'Hiç'}
              </p>
            </div>
              ...
              {fallbackStats.lastUsed
                ? (() => {
                    const days = Math.floor(
                      (Date.now() - new Date(fallbackStats.lastUsed).getTime()) /
                        (1000 * 60 * 60 * 24),
                    );
                    return t('statistics.generic.days_ago', { days }) || `${days} gün önce`;
                  })()
                : t('statistics.generic.no_usage_recorded') || 'Kullanım kaydı yok'}
            </span>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader 
            title={t('statistics.generic.usage_trends_title') || 'Kullanım Trendleri'}
            subtitle={t('statistics.generic.usage_trends_subtitle') || 'Aylık kullanım değişimi'}
          />
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-muted/60 rounded-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t('statistics.generic.uses_count', { count: trend.value }) ||
                        `${trend.value} kullanım`}
                    </p>
                  </div>
                </div>
        ...
        <Card>
          <CardHeader 
            title={t('statistics.generic.top_users_title') || 'En Aktif Kullanıcılar'}
            subtitle={t('statistics.generic.top_users_subtitle') || 'Bu kaydı en sık kullanan kullanıcılar'}
          />
              ...
              <div key={user.userId} className="flex items-center justify-between p-3 hover:bg-muted rounded-lg transition-colors">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t('statistics.generic.user_id_label') || 'Kullanıcı ID'}: {user.userId}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t('statistics.generic.uses_label') || 'kullanım'}
                  </p>
                </div>
              </div>
            ))}
      ...
      <Card>
        <CardHeader 
          title={t('statistics.generic.detailed_metrics_title') || 'Detaylı Metrikler'}
          subtitle={t('statistics.generic.detailed_metrics_subtitle') || 'Kapsamlı kullanım istatistikleri'}
        />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-4">
            <h4 className="text-sm font-medium text-foreground">
              {t('statistics.generic.status_distribution') || 'Durum Dağılımı'}
            </h4>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {t('statistics.generic.active') || 'Aktif'}
                </span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {t('statistics.generic.inactive') || 'Pasif'}
                </span>
                <div className="flex items-center space-x-2">
                  <div className="w-20 h-2 bg-muted rounded-full">
                    <div 
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-medium text-foreground">
              {t('statistics.generic.monthly_activity') || 'Aylık Aktivite'}
            </h4>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {t('statistics.generic.created') || 'Oluşturulan'}
                </span>
                <Badge variant="success" size="sm">{fallbackStats.createdThisMonth}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {t('statistics.generic.updated') || 'Güncellenen'}
                </span>
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-medium text-foreground">
              {t('statistics.generic.performance') || 'Performans'}
            </h4>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {t('statistics.generic.avg_daily_usage') || 'Günlük Ortalama'}
                </span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {t('statistics.generic.peak_usage') || 'En Yüksek'}
                </span>
                <span className="text-sm font-medium text-foreground">
